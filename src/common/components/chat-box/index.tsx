...
import ChatMessageSender from "../chat-sender/index";
import ChatConversationMG from "../chat-sider";
import { ChatCompletionMessageParam } from "openai/resources/index";
import { LLMInfo } from "../chat-bubble-list";
import { mockHistoryMessages } from "@/common/config/mock";
import { getTimeKey, processContent } from "@/utils/utils";
import { LLMCallBackMessage, LLMCallBackMessageChoice, LLMClient } from "aichat-core";
...
const ChatBox: React.FC = () => {
  ...
  /** 初始化用户消息及AI消息 */
  const onInitMessage = (messageId: number, role: string, conversationKey: string, content: string, loading: boolean = false) => {
    setMessageHistory((prev) => ({
      ...prev,
      [conversationKey]: [
  ...
  const onInitMessageContent = (userContent: string): ChatCompletionMessageParam[] => {
   ...
   */
  const onRenameConversation = (conversationKey: string, userContent: string, part: number = 20) => {
    /* 更新会话标题 */
    setConversations((prev) => {
      let item = prev.find(
        (conver) => conver.key === conversationKey
      );
  ...
  const onLLMChatStream = async (messageId: number, conversationKey: string, userContent: string, abortControllerRef: AbortController) => {
    if (llmClient.current && message) {
    ...
    */
    request: async ({ message }, { onError }) => {
      try {
        ...
        // 初始化用户信息
        onInitMessage(getTimeKey(), LLMClient.ROLE_USER, _currentConversation, userContent);
        /** 这个地方一定要 + 1000 * ？毫秒数，以防止和上述用户的消息key在同一时刻相同了（demo的时候复现了好几次） */
        let messageId = getTimeKey(5000);
        // 初始化ai聊天信息，先占个位置
        onInitMessage(messageId, LLMClient.ROLE_AI, _currentConversation, "", true);
        // 发起流式问答(一定要带上messageId，后续展示消息列表时，这个消息ID很有用)
        await onLLMChatStream(messageId, _currentConversation, userContent, abortController.current);
      } catch (error: any) {
  ...
  const onSubmit = (val: string) => {
    if (!val) return;
    if (loading) {
      message.error("当前回答还在进行中，请完成后再发起下一轮问答！");
      return;
    }
